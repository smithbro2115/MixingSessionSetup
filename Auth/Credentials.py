import configparser
from Functionality.useful_utils import get_app_data_folder
from GUI.CustomGUI import LoginDialog
...
def get_credentials(website):
    try:
        ...
        outcome = bool(dialog.exec_())
        credentials = (dialog.ui.userLineEdit.text(), dialog.ui.passwordLineEdit.text()), outcome
        if not outcome:
            raise Canceled
        if dialog.ui.rememberCheckBox.checkState() and outcome:
